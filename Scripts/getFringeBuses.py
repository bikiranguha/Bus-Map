			...
			if NeighbourName.startswith('T3W') or NeighbourName.endswith('M'):
				...
				for d2Neighbour in Depth2Neighbours:
					if d2Neighbour != Bus and float(CAPEBusDataDict[d2Neighbour].NominalVolt) < 345.0:
						...
						if d2Neighbour not in ParentDict.keys():
							ParentDict[d2Neighbour] = set()
							ParentDict[d2Neighbour].add(tfNeighbour)
						else:
							 ParentDict[d2Neighbour].add(tfNeighbour)
				...
				if tfNeighbour not in ParentDict.keys():
					ParentDict[tfNeighbour] = set()
					ParentDict[tfNeighbour].add(Bus)
				else:
					 ParentDict[tfNeighbour].add(Bus)
	...
	if CAPEBusDataDict[Bus].area == '222' and float(CAPEBusDataDict[Bus].NominalVolt) < 345.0:
		...
		for neighbour in neighboursList:
			if CAPEBusDataDict[neighbour].area != '222' :
				# info needed for getting artificial load bus flows
				if Bus not in ParentDict.keys():
					ParentDict[Bus] = set()
					ParentDict[Bus].add(neighbour)
				else:
					 ParentDict[Bus].add(neighbour)
			...
			if neighbour not in Imp138PathSet and neighbour not in directTFConnSet and neighbour not in BoundaryBusSet:
				if float(CAPEBusDataDict[neighbour].NominalVolt) < 345.0:
					...
					if neighbour not in ParentDict.keys():
						ParentDict[neighbour] = set()
						ParentDict[neighbour].add(Bus)
					else:
						 ParentDict[neighbour].add(Bus)
			...
			for tieNeighbour in tieNeighbourList:
				if tieNeighbour not in BusGroup and tieNeighbour not in Imp138PathSet:
					if float(CAPEBusDataDict[tieNeighbour].NominalVolt) < 345.0:
						...
						# info needed for getting artificial load bus flows
						if tieNeighbour not in ParentDict.keys():
							ParentDict[tieNeighbour] = set()
							ParentDict[tieNeighbour].add(tie)
						else:
							ParentBusGroup = CAPEBranchGroupDict[ParentDict[tieNeighbour]]
							if tie in ParentBusGroup:
								continue
							ParentDict[tieNeighbour].add(tie)
...
for Bus in list(ArtificialLoadBusSet):
	BusName = CAPEBusDataDict[Bus].name
	BusVolt = CAPEBusDataDict[Bus].NominalVolt
	string = Bus + ',' + BusName + ',' + BusVolt + ',' + str(ParentDict[Bus])
	if float(BusVolt) > 138.0:
		print string
...
for Bus in list(ArtificialLoadBusSet):
	print Bus + ':' + str(ParentDict[Bus])
# Need to change the following things:
# All the ties to the buses in the important 138 kV paths need to be added as important buses
# The (actual) branches to the remaining buses in the group which are not in the path need to be added as artificial loads
