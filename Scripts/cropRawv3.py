...
# import manual bus data and branch data, tf data has already been subbed
manual_bus_input_file = 'manual_bus_data.txt'
manual_branch_input_file = 'manual_branch_data_new.txt'  # all bus numbers there follow the new format
toBeSubbedBusLines = []
toBeSubbedBranchLines = []
toBeSubbedBusSet = set()  # bus data should be skipped while auto-extraction, they are added at the end manually
toBeSubbedBranchSet = set()
doNotIncludeSet = set() # special buses which should not be included in this cropped raw file

# get all the relevant info from the manual_bus_data file
with open(manual_bus_input_file,'r') as f:
	filecontent = f.read()
	fileLines = filecontent.split('\n')

doNotIncludeStartIndex = fileLines.index("Do not include following buses and associated data:") + 1
doNotIncludeEndIndex = fileLines.index("Replace bus data:")
subStartIndex = fileLines.index("Replace bus data:") + 1 

for i in range(doNotIncludeStartIndex,doNotIncludeEndIndex):
	line = fileLines[i]
	if line == '':
		continue
	doNotIncludeSet.add(line.strip())


for i in range(subStartIndex,len(fileLines)):
	line = fileLines[i]
	if line.strip() == '':
		continue
	words = line.split(',')
	Bus = words[0].strip()
	toBeSubbedBusSet.add(Bus)
	toBeSubbedBusLines.append(line)


# get all the relevant data from branch data
with open(manual_branch_input_file,'r') as f:
	filecontent = f.read()
	fileLines = filecontent.split('\n')
	for line in fileLines:
		...
		Bus2 = words[1].strip()
		cktID = words[2].strip()
		branchID = Bus1.rjust(6) + ',' +  Bus2.rjust(6) + ',' +  cktID
		toBeSubbedBranchSet.add(branchID)
		toBeSubbedBranchLines.append(line)


############################
import sys
#sys.path.insert(0,'C:/Users/Hemanth/OneDrive//Donut Hole v2')
#sys.path.insert(0,'C:/Users/Hemanth/OneDrive//Donut Hole v2/Raw with only 2 winders/Automate 345 kV mapping')
...
CAPERaw = 'new_Raw0706_CCMapsApplied.raw' # most updated merged raw file, with all verified mappings applied
#ArtificialLoadMapFileNew = ['ArtificialLoadMapping.txt', 'ArtificialLoadMapping_Manual.txt'] # input mapping file for artificial loads
ArtificialLoadMapFileNew = 'ArtificialLoadMapFileNew.txt' # file containing updated Artificial Load maps (both auto and manual) with midpoint numbers updated
newRawFile = 'RAWCropped_0711.raw' # new cropped raw file
...
AllToBeMappedFile = 'AllToBeMappedFileUpDated.txt' # input file to AllToBeMappedSet
...
ArtificialLoadBusFile = 'ArtificialLoadBusFileUpdated.txt' # input file to ArtificialLoadBusSet
...
#doNotIncludeSet = ['7', '9', '400001'] # special buses which should not be included in this cropped raw file
...
with open(ArtificialLoadMapFileNew,'r') as f:
	filecontent = f.read()
	fileLines = filecontent.split('\n')
	...
	for line in fileLines:
		...
		CAPEFromBus = CAPEBuses[0].strip() # the artificial load bus
		PlanningBuses = PlanningSide.split(',')
		PlanningFromBus = PlanningBuses[0].strip()
		PlanningToBus = PlanningBuses[1].strip()
		ALLine = artificialLoad(CAPEFromBus, PlanningFromBus, PlanningToBus, planningFlowData)
...
with open(CAPERaw,'r') as f:
	...
	for line in fileLines:
		...
		if Bus in doNotIncludeSet:
			continue
		if Bus in toBeSubbedBusSet: # 345 or higher bus whose bus data has been manually modified
			AllToBeMappedSet.add(Bus)
			continue
		if BusType == '4':
			continue
		...
		if CAPEBusDataDict[Bus].area == '222' and float(CAPEBusDataDict[Bus].NominalVolt) >= 345.0 and Bus not in ArtificialLoadBusSet:
			newBusLines.append(line)
			AllToBeMappedSet.add(Bus)

		elif Bus in AllToBeMappedSet and Bus not in ArtificialLoadBusSet:
			newBusLines.append(line)

# add manual bus data
for line in toBeSubbedBusLines:
	newBusLines.append(line)
...
for i in range(branchStartIndex, branchEndIndex):
	...
	if Bus1 in doNotIncludeSet or Bus2 in doNotIncludeSet:
		continue
	branchID = line[:18]
	#branchID = Bus1.rjust(6) + ',' + Bus2.rjust(6) + ',' + cktID
	if branchID in toBeSubbedBranchSet:
		#print line
		continue
	if status != '1':
		continue
	...
	if Bus1area == '222' and Bus2area == '222': # only include internal comed branches, boundary branches are already included in croppedBranchFile.txt
		if Bus1 in AllToBeMappedSet and Bus1 not in ArtificialLoadBusSet: # Important from bus
			if Bus2 in AllToBeMappedSet and Bus2 not in ArtificialLoadBusSet: # discard the branch line where the to bus data has been incorporated into artificial loads
				newBranchLines.append(line)
		elif Bus2 in AllToBeMappedSet and Bus2 not in ArtificialLoadBusSet: # Important from bus
			if Bus1 in AllToBeMappedSet and Bus1 not in ArtificialLoadBusSet: # discard the branch line where the to bus data has been incorporated into artificial loads
				newBranchLines.append(line)
# add manual branch data
for line in toBeSubbedBranchLines:
	newBranchLines.append(line)
...
"""
for key in loadCktIDDict.keys():
	cktID  = loadCktIDDict[key]
	if cktID > 1:
		print key
"""
...
for line in artificialLoadLines:
	...
	cktID = loadCktIDDict[Bus]
	cktIDStr = "'" + str(cktID) + " '"
	words[1] = cktIDStr
	line = reconstructLine2(words)
	loadCktIDDict[Bus] -=1 # decrease ckt id for next load, if any

	"""
	# if the new artificial load bus had loads before, we need to increment the circuit id while adding the load data
	try:
