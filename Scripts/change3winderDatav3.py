...
def tryMidpoints(Bus,MidpointNeighbour,MidpointDict,currentBusList,currentSet,PSList):
		...
		# case 2: found exact match for three winding tf in planning data
			...
			if notFound == True: # three winder equivalent tf not found, append 2 lines
				...
				if planningBus1 in MidpointNeighbour.keys():
					...
					skip = tryMidpoints(planningBus1,MidpointNeighbour,MidpointDict,currentBusList,currentSet,PSList)
					if skip == True:
					...
					PSList = getPhaseShifts(i, fileLines)
					skip = tryMidpoints(planningBus2,MidpointNeighbour,MidpointDict,currentBusList,currentSet,PSList)
					if skip == True:
						toChange3winder[key3winderMapLog] = str(currentSet)
					...
					PSList = getPhaseShifts(i, fileLines)
					skip = tryMidpoints(planningBus3,MidpointNeighbour,MidpointDict,currentBusList,currentSet,PSList)
					if skip == True:
						toChange3winder[key3winderMapLog] = str(currentSet)
