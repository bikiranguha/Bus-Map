...
#ManualMapFile = 'test_branch_comparisonMult.txt'
ManualMapFile = 'test_branch_comparisonMultTest.txt'
automated345MapFile = 'automated345MapFileTest.txt'




# each key of autoMapDict will be this structure
class mapStruct(object):
	# structure which keeps track of the maps and also the parent map (original map from which this map was determined)
	def __init__(self):
		self.map = set()
		self.planningParent = 'root'
		self.CAPEParent = 'root'
##############
...
BigAutoMapDict = {} # dict whose each element is a autoMapDict, key is the input map bus
#################################
	...
	if len(potentialMaps) == 1:
		...
		try:
			...
			if CAPEneighbour in SubStationDictNew[planningSubStation]: # names match and impedances match
				...
				autoMapDict[CAPEneighbour] = mapStruct()
				autoMapDict[CAPEneighbour].map.add(planningNeighbour)
				autoMapDict[CAPEneighbour].planningParent = planningBus
				autoMapDict[CAPEneighbour].CAPEParent = originBus
				...
				if planningNeighbour in BranchGroupDictPlanning.keys():
					...
					for planningTie in planningTies:
						autoMapDict[CAPEneighbour].map.add(planningTie)
...
# Loop through all the inputs given
for mappedBus in ManualMappingDict.keys(): # mappedBus is the CAPE map bus which is given as input to script
	autoMapDict = {}
	autoMappedSet = set()
	explored = set()
	planningExplored = set()
	...
	frontier.put(mappedBus) # Frontier will add only those CAPE buses which have a verified mapping
	#autoMapDict = dict(ManualMappingDict)

	# get any ties of the input planning bus
	originPlanningBus = ManualMappingDict[mappedBus][0] # the planning bus which is mapped to the CAPE bus given in the input file
	if originPlanningBus in BranchGroupDictPlanning.keys(): # if the origin planning bus has any ties, map the set to the CAPEBus
		...
		autoMapDict[mappedBus] = mapStruct()
		for planningTie in originPlanningTies:
			autoMapDict[mappedBus].map.add(planningTie)
		#autoMapDict[mappedBus] = set(originPlanningTies)
	else:
		autoMapDict[mappedBus] = mapStruct()
		autoMapDict[mappedBus].map.add(originPlanningBus)
		#autoMapDict[mappedBus] = set()
		#autoMapDict[mappedBus].add(originPlanningBus)
	...
	print 'Starting CAPE bus of this mapping loop: ' + mappedBus
	print '\n'
	...
	while not frontier.empty():
		...
		planningBusSet = autoMapDict[currentBus].map # group of planning buses, if they are part of ties
		currentBusGroup = set() # list of buses belonging to the tie group 
		currentBusGroup.add(currentBus)


		# map all ties to the same planning set
		planningBus = list(planningBusSet)[0]
			...
			for tie in CAPEBranchTies:
				if tie != currentBus:
					...
					if tie not in explored:
						...
						#explored.add(tie)
						###
						autoMapDict[tie] = mapStruct()
						autoMapDict[tie].map.add(planningBus)
						autoMapDict[tie].CAPEParent = currentBus + ' tie '
						autoMapDict[tie].planningParent = planningBus + ' tie '
						"""
						autoMapDict[tie] = set()
						...
						if planningBus in BranchGroupDictPlanning.keys():
							...
							for planningTie in planningTies:
								autoMapDict[tie].map.add(planningTie)
	...
	BigAutoMapDict[mappedBus] = autoMapDict
######################################
...
with open('conflictsMoreDetails.txt','w') as f: 
	for key in BigAutoMapDict.keys():
		currentMapDict = BigAutoMapDict[key]
		for nestedKey in currentMapDict.keys():
			if nestedKey not in MergedMapdict.keys(): # 1st encounter of this CAPE bus
				MergedMapdict[nestedKey] = currentMapDict[nestedKey]
			else: # Not 1st encounter
				OldMapSet = MergedMapdict[nestedKey].map
				NewMapSet = currentMapDict[nestedKey].map
				if OldMapSet != NewMapSet: # previous map set of CAPE bus was different
					currentStr = 'Conflict for mapping of Bus ' + nestedKey
					f.write(currentStr)
					f.write('\n')
					currentStr = 'Previous Map Set: ' + str(OldMapSet) + ' PlanningParent: ' + MergedMapdict[nestedKey].planningParent + ' CAPEParent: ' + MergedMapdict[nestedKey].CAPEParent
					f.write(currentStr)
					f.write('\n')
					currentStr = 'New Map Set: ' + str(NewMapSet) + ' PlanningParent: ' + currentMapDict[nestedKey].planningParent + ' CAPEParent: ' + currentMapDict[nestedKey].CAPEParent
					f.write(currentStr)
					f.write('\n')


print 'Number of buses mapped ' + str(len(MergedMapdict.keys()))

# output the maps to a file
with open(automated345MapFile,'w') as f:
	f.write('CAPEBus->PlanningBusList')
	for key in MergedMapdict.keys():
		planningMapList = list(MergedMapdict[key].map)
		string = key + '->' # CAPEBus

		for planningMap in planningMapList:  # append all the planning buses to the map string
			string += planningMap
			string += ','
		string = string[:-1]
