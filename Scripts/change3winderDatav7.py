	...
	if Bus3 != '0':
		...
		# case 2: found exact match for three winding tf in planning data
			...
			if len(set(planningBusList)) <3: # all the mapping is not unique
				...
				if len(set(planningBusList)) == 1: # all three buses mapped to the same planning bus
					Bus2 = currentBusList[1]
					Bus3 = currentBusList[2]
					Bus2V = float(CAPEVoltageDict[Bus2])
					Bus3V = float(CAPEVoltageDict[Bus3])
					if Bus2V > 10.0 or Bus3V >10.0:
						out = generateIssueString(currentBusList,planningBusList)
						#print out
					#UATBusSet.add(Bus2)
					#UATBusSet.add(Bus3)
					...
					if planningBus1 in tfNeighbourDict.keys():
						if planningBus2 not in tfNeighbourDict[planningBus1]:		
							string = generateIssueString(currentBusList,planningBusList)
							tfIssueLines.append(string)
