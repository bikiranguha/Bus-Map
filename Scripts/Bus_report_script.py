...
with open(CAPERaw, 'r') as f:
	...
	for line in fileLines:
		...
		Bus = words[0].strip()
		area = words[4].strip()
		volt = float(words[2].strip())

		if area == '222' and volt >= 345.0 :
			ComedBusSet345.add(Bus)
...
psse_log = 'BusReports_RAW0606_final.txt'
redirect.psse2py()
...
for bus in ComedBusSet345:
	ierr = psspy.bsys(1,0,[0.0,0.0],0,[],1,[int(bus)],0,[],0,[]) # PAGE 1373 of API book
	ierr = psspy.pout(1,0) # page 258 of API book
