...
ArtificialLoadMappingFileList = ['ArtificialLoadMapping.txt', 'ArtificialLoadMapping_Manual.txt'] # input mapping file for artificial loads
...
AllToBeMappedFile = 'AllToBeMappedFile.txt' # input file to AllToBeMappedSet
...
for file in ArtificialLoadMappingFileList:
...
for i in range(branchStartIndex, branchEndIndex):
	...
	if Bus1area == '222' and Bus2area == '222': # only include internal comed branches, boundary branches are already included in croppedBranchFile.txt
		if Bus1 in AllToBeMappedSet and Bus1 not in ArtificialLoadBusSet: # Important from bus
			if Bus2 not in ArtificialLoadBusSet: # discard the branch line where the to bus data has been incorporated into artificial loads
				newBranchLines.append(line)
		elif Bus2 in AllToBeMappedSet and Bus2 not in ArtificialLoadBusSet: # Important from bus
			if Bus1 not in ArtificialLoadBusSet: # discard the branch line where the to bus data has been incorporated into artificial loads
				newBranchLines.append(line)
