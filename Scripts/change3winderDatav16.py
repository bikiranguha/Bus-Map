...
from sortBusDataVoltage import sortBusData #  function to sort any data
...
AllMappedBusDataOld = 'AllMappedBusDataOld.txt'
AllMapFileSorted = 'AllMappedBusDataSorted.txt' # All mapped bus data, without midpoint buses and non-shifted bus angles
...
#newBusAnglesSorted = 'newBusAnglesSorted.txt' # file containing changed bus angle list
...
newBusAnglesSorted = directory +  'newBusAnglesSorted.txt' # file containing changed bus angle list
...
angleChangeLines = []
DownStreamShiftSet =set() # set of all LV buses which got their phase shifted because their primary side got phase shifted
...
while i < tfEndIndex:
	...
	if Bus1 in CAPEComedBusSet: # only check further if the transformer belongs to comed
		...
		if Bus3 != '0':
			...
			if float(PSList[0]) != 0.0:
				#if float(CAPEVoltageDict[Bus1]) > float(CAPEVoltageDict[Bus2]):
				#	print CAPEVoltageDict[Bus1], CAPEVoltageDict[Bus2]
	...
	else: # transformer does not belong to comed
		if Bus3 == '0':
			i+=4
		else:
			i+=5
...
sortBusData(AllMappedBusDataOld,AllMapFileSorted)

with open(AllMapFileSorted,'r') as f:
	filecontent = f.read()
	fileLines = filecontent.split('\n')


	# start applying the phase shifts
	for line in fileLines:
		words = line.split(',')
		if len(words) <2:
			continue
		...
		if Bus in UATBusSet: # bus needs to have angle changed
			if Bus in tf2wNeighbourDictCAPE.keys():
				# shift angles for any LV buses downstream
				LVtfConn = [x for x in tf2wNeighbourDictCAPE[Bus] if (CAPEVoltageDict[Bus] > CAPEVoltageDict[x])] # get any LV buses downstream
				if len(LVtfConn)>0: 
					for conn in LVtfConn:
						if conn not in DownStreamShiftSet: # conn already had angles shifted by this same method
							if conn in UATAngleDict.keys():
								UATAngleDict[conn] +=UATAngleDict[Bus]
							else:
								UATAngleDict[conn] = UATAngleDict[Bus]
							DownStreamShiftSet.add(conn)
						else: # Downstream shift already applied to this bus, skip further downstream shift
							print 'Downstream shift already applied to this bus, skip further downstream shift: ', conn
		...
		elif Bus in DownStreamShiftSet: # Downstream shift bus, need to add phase shift
			Angle = words[8].strip()
			newAngle = float(Angle) + UATAngleDict[Bus]
			words[8] = ' '*(9- len(str(newAngle))) + str(newAngle)
			nLine = reconstructLine2(words)
...
with open(newBusAnglesSorted,'w') as f:
...
sortBusData(newBusAnglesSorted,newBusAnglesSorted)
...
print 'Set of buses where downstream shift has been applied: '
print DownStreamShiftSet
