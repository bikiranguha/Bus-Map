...
NameMatchDictCAPE = {}
"""
class BusNameData(object):
	def __init__(self):
		self.Name = ''
		self.PossibleMapBusNo = []
		self.PossibleMapBusName = []
"""
...
for planningBusName in NameMatchDictPlanning.keys():
	for CAPEsubName in subNameSet:
		...
		if similarity > 0.5:
			NameMatchDictPlanning[planningBusName].append(CAPEsubName)

with open('NameMatchResults.txt','w') as f:
	for planningBusName in NameMatchDictPlanning.keys():
		string = planningBusName + ':' + str(NameMatchDictPlanning[planningBusName])
		f.write(string)
		f.write('\n')
