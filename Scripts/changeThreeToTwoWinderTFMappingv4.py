...
#planningtfFoundList = [] # list of all generated transformer mappings which do not exist in planning case
#planningtfNotFoundDict = {}
#planningtfNotFoundSet = set()
...
with open(OldtfReplaceFile,'r') as f:
	...
	for line in fileLines:
		...
		# check if the generated planning tf actually exists in planning case
		planningtfFound = 0
		...
		if planningtfFound == 1: # generated planning tf exists, make a new line and add to lines list
		# generate new lines
		...
		else: # generated transformer does not exist in planning, check in OldtfReplaceFile
			#newLine = CAPESide + '->' + planningSide
			planningPart = words[1].split(',')
			oldPlanningBus1 = planningPart[0].strip()
			oldPlanningBus2 = planningPart[1].strip()

			oldPlanningtfFound = 0
			oldPlanningSide = oldPlanningBus1+','+oldPlanningBus2

			for tf in planningtfList:
				if oldPlanningSide in tf:
					oldPlanningSide = str(tf)
					oldPlanningtfFound = 1
					break

			if oldPlanningtfFound == 1: # OldtfReplaceFile mapping holds true in this case, add this to tf maps
				newLine = CAPESide + '->' + oldPlanningSide
				newMapLines.append(newLine)
			else: # cannot find equivalent tf anywhere, print line
				newLine = CAPESide + '->' + oldPlanningSide
				print newLine



			#planningtfNotFoundSet.add(planningSide)
			#planningtfNotFoundDict[CAPESide] = planningSide
